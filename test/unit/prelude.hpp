...
inline auto mag_tensor_buf_e8m23_to_vec(const mag_tensor_t* tensor, std::vector<mag_e8m23_t>& out) -> void {
    mag_assert2(tensor->dtype == MAG_DTYPE_E8M23);
    out.clear();
    out.reserve(mag_tensor_numel(tensor));
    std::memcpy(out.data(), mag_tensor_data_ptr(tensor), mag_tensor_data_size(tensor));
}

inline auto mag_tensor_buf_e5m10_to_vec(const mag_tensor_t* tensor, std::vector<mag_e5m10_t>& out) -> void {
    mag_assert2(tensor->dtype == MAG_DTYPE_E5M10);
    out.clear();
    out.reserve(mag_tensor_numel(tensor));
    std::memcpy(out.data(), mag_tensor_data_ptr(tensor), mag_tensor_data_size(tensor));
}
