...
static auto mm_naive(const mag_e8m23_t* A, const mag_e8m23_t* B, mag_e8m23_t* C, const std::int64_t M, const std::int64_t N,
                     const std::int64_t K) -> void {
    for (std::int64_t i = 0; i < M * N; ++i)
        C[i] = 0.0f;
    for (std::int64_t i = 0; i < M; ++i) {
        for (std::int64_t k = 0; k < K; ++k) {
            mag_e8m23_t a_ik = A[i * K + k];
...
TEST(matmul_cpu, mm_naive_verify) {
    static constexpr mag_e8m23_t A[6] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f};
    static constexpr mag_e8m23_t B[2] = {0.5f, -1.0f};
    mag_e8m23_t C[3];
...
TEST(matmul_cpu, mm_square_2x2) {
    ...
    static constexpr mag_e8m23_t A_data[M][N] = {{1.6354027, -1.3607267}, {1.8556793, 1.1689897}};
    static constexpr mag_e8m23_t B_data[M][N] = {{-0.6105532, 0.10695228}, {-1.0069681, -0.40955952}};
    ...
    static constexpr mag_e8m23_t expected[M][N] = {0.3717081, 0.7322086, -2.3101263, -0.28030172};
    auto* buf = static_cast<const mag_e8m23_t*>(mag_tensor_data_ptr(R));
    for (std::int64_t i = 0; i < M; ++i)
        for (std::int64_t j = 0; j < N; ++j)
...
TEST(matmul_cpu, mm_square_2x2_transpose_x) {
    ...
    static constexpr mag_e8m23_t expected[M * N] = {-2.8671103f, -0.58510107f, -0.3463393f, -0.6243037f};
    auto* buf = static_cast<const mag_e8m23_t*>(mag_tensor_data_ptr(R));
    for (std::int64_t i = 0; i < M * N; ++i)
...
TEST(matmul_cpu, mm_square_2x2_transpose_y) {
    ...
    constexpr std::size_t M = 2, N = 2;
    static constexpr mag_e8m23_t A_data[M][N] = {
        {1.6354027, -1.3607267},
        {1.8556793, 1.1689897}
    };
    static constexpr mag_e8m23_t B_data[M][N] = {
    ...
    static constexpr mag_e8m23_t expected[M*N] = {
...
TEST(matmul_cpu, mm_rect_2x3_3x4) {
    ...
    static constexpr mag_e8m23_t A_data[M][K] = {{1.0f, 2.0f, 3.0f}, {4.0f, 5.0f, 6.0f}};

    static constexpr mag_e8m23_t B_data[K][N] = {
        {1.0f, 2.0f, 3.0f, 4.0f}, {5.0f, 6.0f, 7.0f, 8.0f}, {9.0f, 10.0f, 11.0f, 12.0f}};

    static constexpr mag_e8m23_t expected[M][N] = {{38.0f, 44.0f, 50.0f, 56.0f}, {83.0f, 98.0f, 113.0f, 128.0f}};
    ...
    const mag_e8m23_t* buf = static_cast<const mag_e8m23_t*>(mag_tensor_data_ptr(R));
...
TEST(matmul_cpu, mm_matrix_vector_2x3) {
    ...
    static constexpr mag_e8m23_t v_data[K] = {7.0f, 8.0f, 9.0f};
    static constexpr mag_e8m23_t expected[M] = {50.0f, 122.0f};
...
TEST(matmul_cpu, mm_vector_matrix_3x2) {
    ...
    static constexpr mag_e8m23_t v_data[1][K] = {{1.0f, 2.0f, 3.0f}};

    static constexpr mag_e8m23_t B_data[K][N] = {{4.0f, 5.0f}, {6.0f, 7.0f}, {8.0f, 9.0f}};

    static constexpr mag_e8m23_t expected[1][N] = {{40.0f, 46.0f}};
