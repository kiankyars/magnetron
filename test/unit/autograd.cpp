...
TEST(autograd, bin_ops1) {
    ...
    // check backward pass
    float gx = mag_tensor_get_scalar_virtual_index(x->grad, 0);
    float gy = mag_tensor_get_scalar_virtual_index(y->grad, 0);
    float gz = mag_tensor_get_scalar_virtual_index(z->grad, 0);
