...
def tonumpy(t: Tensor, dtype) -> np.array:
...
def binary_op_square(dtype: DType, f: callable, lim: int = 4) -> None:
    ...
    def compute(shape: tuple[int, ...]) -> None:
        ...
        r = f(x, y)
        np.testing.assert_allclose(tonumpy(r, numpy_dt), f(tonumpy(x, numpy_dt), tonumpy(y, numpy_dt)), atol=EPS)
...
def unary_op(
    ...
    def compute(shape: tuple[int, ...]) -> None:
        ...
        r = magf(x.clone())
        np.testing.assert_allclose(tonumpy(r, numpy_dt), npf(tonumpy(x, numpy_dt)), atol=EPS)
