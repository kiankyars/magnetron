...
class PerformanceInfo:
    ...
    def plot(self, flops_per_op: int=2, plot_style: str='bars'):
        ...
        if plot_style == 'bars':
            ...
            for i, participant in enumerate(self.participants):
                ...
                gflops = [(elements * flops_per_op) / (time * 1e9)
                         for elements, time in zip(total_elements, participant.timings)]
                ax2.plot(dims, gflops, label=participant.name,
                        color=colors[i%len(colors)], marker=markers[i%len(markers)])
...
def benchmark(name: str, participants: list[BenchParticipant], func: callable, 
             shapes: list[tuple[tuple[int, int], tuple[int, int]]], iters: int=10000,
             plot_style: str='bars') -> PerformanceInfo:
    shapes_a = []
    shapes_b = []
